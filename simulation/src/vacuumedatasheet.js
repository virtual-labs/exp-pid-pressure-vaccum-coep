...
$("#datasheetBody").html(htm);
      ...
      for (let i = 0; i < dataAr1.length; i++) {
    	  // Add tab
    	  ...
    	  let rows = ` `;
    	  for (let j = 0; j < dataAr1[i].data.length; j++) {
    	    rows += `
    	      <tr>
    	        <td>${j + 1}</td>
    	        <td>${dataAr1[i].data[j].s1}</td>
    	        <td>${dataAr1[i].data[j].s2}</td>
    	        <td>${dataAr1[i].data[j].s3}</td>
    	        <td>${dataAr1[i].data[j].s4}</td>
    	        <td>${dataAr1[i].data[j].s5}</td>
    	      </tr>`;
    	  }
      ...
      document.getElementById('generatePdf').addEventListener('click', function () {
    	    ...
    	    doc.save('Vacuum_sensor_test_cycles.pdf'); // Download the generated PDF
    	  });
}
